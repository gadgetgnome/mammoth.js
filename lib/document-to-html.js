...
function DocumentConversion(options, comments) {
    ...
    function getListType(data) {
        const fmt = data.numFmt?.toLowerCase() || "";
        const lvlText = data.lvlText || "";
        ...
        if (fmt === "bullet") {
            return lvlText === "-" ? "strek" : "bullet";
        }

        if (letterFormats.includes(fmt)) {
            return lvlText === "%1." ? "alpha-d" : "alpha";
        }

        if (numberFormats.includes(fmt)) {
            return "number";
        }

        console.log("Unhandled numFmt:", fmt);
        return "number";
    }
